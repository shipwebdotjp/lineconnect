...
use Shipweb\LineConnect\Trigger\Webhook as TriggerWebhook;
...
class Webhook {
	...
	/**
	 * REST API callback for WP route.
	 *
	...
	public static function rest_callback( \WP_REST_Request $request ) {
		...
		$sig  = $request->get_header( 'x-line-signature' );
		if ( $sig ) {
			// set so signature verification code can use the same global
			$_SERVER['HTTP_X_LINE_SIGNATURE'] = $sig;
		}

		return self::handle( $body, false );
	}

	/**
	 * Main handler. When $is_legacy is true the handler will behave like the
	 * original bot.php (send low level HTTP response and exit). When false it
	 ...
	 */
	public static function handle( $json_string = null, $is_legacy = true ) {
		if ( $json_string === null ) {
		...
		foreach ( LineConnect::get_all_channels() as $channel_id => $channel ) {
			...
			$XLineSignature = isset( $_SERVER['HTTP_X_LINE_SIGNATURE'] ) ? $_SERVER['HTTP_X_LINE_SIGNATURE'] : '';
		...
		if ( ! isset( $json_obj->{'events'} ) ) {
			if ( $is_legacy ) {
				exit;
			}
			return new \WP_REST_Response( array( 'status' => 'no_events' ), 200 );
		}
		...
		foreach ( $json_obj->{'events'} as $event ) {
			...
			if ( $type === 'message' ) {    // メッセージ受け取り時
				...
				if ( $link_obj->{'result'} === 'ok' ) {
					...
					$user_id = get_transient( 'lineconnect_nonce' . $nonce );
					...
					if ( $user_id ) {
						...
						// nonceを削除
						delete_transient( 'lineconnect_nonce' . $nonce );

						// WordPressユーザーのメタ情報にLINEユーザーIDを追加
						$line_user_data = UserProvider::get_user_meta( $user_id, LineConnect::META_KEY__LINE, true );
						...
						if ( isset( $profile['pictureUrl'] ) ) {
							$line_user_data[ $secret_prefix ]['pictureUrl'] = $profile['pictureUrl'];
						}
						UserProvider::update_user_meta( $user_id, LineConnect::META_KEY__LINE, $line_user_data );
			...
			foreach ( $triggers as $trigger ) {
				...
				foreach ( $trigger['triggers'] as $trigger_item ) {
					$matched_array[] = TriggerWebhook::check_trigger_condition( $trigger_item, $event, $secret_prefix );
				}
			...
			if ( ! empty( $message ) && ! empty( $reply_token ) ) {
				...
				self::LoggingAPIResponse( $resp, $message );
			}
	...
	public static function LoggingAPIResponse( $response, $message ) {
