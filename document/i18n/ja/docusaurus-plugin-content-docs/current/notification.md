...
3. 「送信対象」リストからLINEで通知するユーザーを選択します。
   - 各チャネルでは「すべての友達」「連携済みの友達」、および連携済みの友達のうち各ロールに属するユーザーを選択できます。（複数選択可能）
   - リストの最後に「オーディエンス」が表示されます。オーディエンスを選択した場合は、WordPress のカスタム投稿タイプ「Audience」に保存されている投稿一覧が選択肢として表示されます。複数のオーディエンスにチェックを入れると、それぞれのオーディエンスに対して個別に送信されます（重複しているユーザーには重複して送信されます）。
...
注意:
- オーディエンスを使った送信は、管理画面の「Audience」として保存済みのカスタム投稿を参照して送信対象を決定します。管理や作成方法はドキュメントの [オーディエンス](./audience.md) セクションを参照してください。
