...
class lineconnectChat {
	...
	static function set_plugin_menu() {
		...
		$page_hook_suffix = add_menu_page(
			...
			// メニュータイトル：
			__('LINE Chat', lineconnect::PLUGIN_NAME),
			// 権限：
	...
	static function show_chat() {
		...
		$toTypeList = array(
			array("name" => "broad", "label" => __('All', lineconnect::PLUGIN_NAME)),
			array("name" => "linked", "label" => __('Linked', lineconnect::PLUGIN_NAME)),
			array("name" => "role", "label" => __('Roles', lineconnect::PLUGIN_NAME)),
			array("name" => "multi", "label" => __('Users', lineconnect::PLUGIN_NAME)),
		);
...
EOM;
						...
						if (!empty($to)) {
							...
							if (!empty($users)) {	//マッチするユーザーが見つかれば
								...
								if ($response['success']) {
									...
									$error_message = __('Failed to send a LINE message to selected users. ', lineconnect::PLUGIN_NAME) . $response['message'];
								}
							} else {
							...
							$error_message = __('No user selected.', lineconnect::PLUGIN_NAME);
						}
					} else {
						if ($type == 'linked') {
							$role = 'slc_linked';
	...
	static function wpdocs_plugin_admin_scripts() {
        ...
        */
		...
		wp_enqueue_script(lineconnect::PLUGIN_PREFIX . 'chat', plugins_url($chat_js, dirname(__FILE__)), array('wp-element', 'wp-i18n'), filemtime(plugin_dir_path(dirname(__FILE__)) . $chat_js), true);
		//JavaScriptの言語ファイル読み込み
		wp_set_script_translations(lineconnect::PLUGIN_PREFIX . 'chat', lineconnect::PLUGIN_NAME, plugin_dir_path(dirname(__FILE__)) . 'languages');
