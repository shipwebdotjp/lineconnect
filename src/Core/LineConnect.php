...
use Shipweb\LineConnect\Admin\UserColumnCustomizer;
...
use Shipweb\LineConnect\Bot\RESTAPI as BotRESTAPI;
...
class LineConnect {
	 ...
	 * 画面のslug：GOTO LOGIN
	 */
	const SLUG__GOTO_LOGIN = self::PLUGIN_ID . '-goto-login';

	/**
	 * 画面のslug：Account Link
	 */
	const SLUG__ACCOUNT_LINK = self::PLUGIN_ID . '-account-link';
	 ...
	 * Filter hook prefix
	 */
	const ACTION_PREFIX = self::PLUGIN_ID . '_action_';
	...
	private function __construct() {
		...
		// プラグイン無効化時に呼び出し
		register_deactivation_hook( $this->root_dir . self::PLUGIN_ENTRY_FILE_NAME, array( $this, 'pluginDeactivation' ) );
	}
	...
	public function init() {
		...
		if ( is_admin() && is_user_logged_in() && ( is_super_admin() || current_user_can( 'administrator' ) ) ) {
			...
			add_action( 'admin_post_' . self::SLUG__AUDIENCE_DOWNLOAD, array( AudienceColumn::class, 'download_audience_page' ) );
			// コンテンツダウンロードアクション
			add_action( 'admin_post_' . self::SLUG__CONTENT_DOWNLOAD, array( ContentDownload::class, 'download_content_page' ) );
			// インタラクションセッションダウンロードアクション
			add_action( 'admin_post_' . self::SLUG__INTERACTION_SESSION_DOWNLOAD, array( InteractionSessionDownload::class, 'download_interaction_session_page' ) );
			// GOTO LOGINアクション
			add_action( 'admin_post_' . self::SLUG__GOTO_LOGIN, array( Account::class, 'goto_login_page' ) );
			add_action( 'admin_post_nopriv_' . self::SLUG__GOTO_LOGIN, array( Account::class, 'goto_login_page' ) );

			// Account Linkアクション
			add_action( 'admin_post_' . self::SLUG__ACCOUNT_LINK, array( Account::class, 'account_link_page' ) );
			add_action( 'admin_post_nopriv_' . self::SLUG__ACCOUNT_LINK, array( Account::class, 'account_link_page' ) );
			...
			add_filter( 'posts_clauses', array( InteractionColumn::class, 'posts_clauses_for_interaction_list' ), 10, 2 );
		...
		add_action( 'slc_action_redirect_account_link', array( $this, 'redirect_account_link' ), 10, 2 );
		...
		// add shedule action
		add_action( self::CRON_EVENT_NAME, array( Cron::class, 'schedule_event' ) );
	...
	*/
	function redirect_account_link( $user_login, $current_user ) {
		// error_log("redirect_account_link called: " . $_COOKIE['line_connect_redirect_to']);
		// error_log("current_user_id: " . UserProvider::get_current_user_id());
		if ( isset( $_COOKIE['line_connect_redirect_to'] ) && UserProvider::get_current_user_id() ) { // COOKIEにリダイレクト先がセットされており、ログイン済みなら
	...
	static function get_wpuser_from_line_id( $secret_prefix, $line_id ) {
		$is_altanative = apply_filters( self::FILTER_PREFIX . 'use_alternative_user_provider', false );
		// error_log( 'get_wpuser_from_line_id: is_altanative:' . ( $is_altanative ? 'true' : 'false' ) );
		if ( $is_altanative ) {
			return apply_filters( self::FILTER_PREFIX . 'get_user_from_line_id', false, $secret_prefix, $line_id );
		}
		/*
		$args       = array(
			'meta_query' => array(
		...
		*/
		...
		if ( ! empty( $userids ) ) {   // マッチするユーザーが見つかれば
			...
			foreach ( $userids as $userid ) {
				$user_meta_line = UserProvider::get_user_meta( $userid, self::META_KEY__LINE, true );
	...
	public static function get_line_id_from_wpuser( $user, $secret_prefix ) {
		return self::get_line_id_from_wpuserid( $user->ID, $secret_prefix );
	}

	/**
	 * WordPressユーザーIDとチャネルからLINEユーザーIDを取得
	 *
	 * @param int    $user_id WordPressユーザーID
	 * @param string $secret_prefix チャネルシークレット先頭4文字
	 * @return string LINEユーザーID
	 */
	public static function get_line_id_from_wpuserid( $user_id, $secret_prefix ) {
		$line_user_id   = null;
		$user_meta_line = UserProvider::get_user_meta( $user_id, self::META_KEY__LINE, true );
		if ( $user_meta_line && isset( $user_meta_line[ $secret_prefix ] ) ) {
	...
	public static function get_line_ids_from_wpuser( $user ) {
		return self::get_line_ids_from_wpuserid( $user->ID );
	}

	/**
	 * WordPressユーザーIDからLINEユーザーIDを取得
	 *
	 * @param int $user_id WordPressユーザーID
	 * @return array LINEユーザーID
	 */
	public static function get_line_ids_from_wpuserid( $user_id ) {
		$line_user_ids  = array();
		$user_meta_line = UserProvider::get_user_meta( $user_id, self::META_KEY__LINE, true );
