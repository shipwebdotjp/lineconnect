...
class AudienceLinkTest extends WP_UnitTestCase {
    ...
    public function test_連携スターテスによるオーディエンス取得() {
        ...
        $linked_line_ids = lineconnectAudience::get_audience_by_condition($condition_linked['condition']);
        $unlinked_line_ids = lineconnectAudience::get_audience_by_condition($condition_unlinked['condition']);
        $all_line_ids = lineconnectAudience::get_audience_by_condition($condition_all['condition']);
        $broadcast_line_ids = lineconnectAudience::get_audience_by_condition($condition_broadcast['condition']);
