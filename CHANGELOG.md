...
- GPT Image 2.0を使用した画像生成アクションを追加。LINEチャットから自然言語でお好みの画像生成を行えます。
- 送信した画像にリプライすることで、その画像について質問したり、その画像をもとに画像編集を行えるようになりました(GPT Image 2.0対応)

## [4.5.0] - 2025-10-02
### Changes
- トリガータイプがスケジュールの場合、オーディエンスを設定できるようになりました。  
 これにより、指定時刻に、特定のオーディエンスに対してアクションを実行することができるようになります。 
 これまでも「オーディエンスを指定してLINEメッセージを送信」アクションを用いることで、特定のオーディエンスに対してLINEメッセージを送信することは可能でしたが、メッセージ送信以外のアクションを実行することができませんでした。
 今回の変更により、例えば、特定のオーディエンスに対して定期的にシナリオを開始したり、リッチメニューを変更したり、ユーザーメタやタグを更新したりすることができるようになります。
- アクションの並べ替え順をわかりやすいように変更しました。
