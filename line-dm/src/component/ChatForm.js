...
const ChatForm = () => {
    ...
    // const [title, setTitle] = useState(lc_initdata['reminder_title']);
    // const [reminder, setReminder] = useState(lc_initdata['reminder_body']);
    const [to, setTo] = useState(lc_initdata['line_id']);
    // const [type, setType] = useState(lc_initdata['dmType']);
    ...
    // const toTypeList = lc_initdata['dmTypeList'];   //メッセージorリマインダー タイプ一覧
    // const roleList = lc_initdata['roleList'];   //ロール一覧
    // const toUsers = lc_initdata['toDatas']; //宛先ユーザー一覧
...
/*     function handleTypeChange(text) {
    ...
    function handleChannelChange(text) {
        setChannel(text);
    ...
    */
    ...
    const handleSubmit = (e) => {
        ...
        jQuery.ajax({
            ...
            data: {
                'action': 'lc_ajax_dm_send',
                'nonce': lc_initdata['ajax_nonce'],
                'to': 'line_id' in to ? to['line_id'] : '',
                'channel': channel,
                'messages': [{
                    "type":"message",
                    "text":message
        ...
        <form onSubmit={handleSubmit}>
            <div className="ChatBody w-1/3">
                <div className="ChatRow">
                    <ChatChannel handleChannelChange={setChannel} channelCheked={channel} channelList={channelList} />
                </div>
                <div className="ChatRow">
                    <ChatTo handleToChange={setTo} toValue={to} />
                </div>
                <div className="ChatRow">
