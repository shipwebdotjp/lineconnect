...
EOM;
					...
					foreach ($tab_details['fields'] as $option_key => $option_details) {
						...
						} else if ($option_key == 'openai_enabled_functions') {
...
EOM;
	...
	static function wpdocs_plugin_admin_scripts() {
		...
		wp_enqueue_script(lineconnect::PLUGIN_PREFIX . 'admin', plugins_url($setting_js, dirname(__FILE__)), array('jquery-ui-tabs', 'wp-color-picker', 'jquery-ui-multiselect-widget', 'wp-i18n'), filemtime(plugin_dir_path(dirname(__FILE__)) . $setting_js), true);

		//JavaScriptの言語ファイル読み込み
		wp_set_script_translations(lineconnect::PLUGIN_PREFIX . 'admin', lineconnect::PLUGIN_NAME, plugin_dir_path(dirname(__FILE__)) . 'languages');
	}
