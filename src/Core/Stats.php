...
/**
 * LINEメッセージ統計情報を管理・取得するクラス
 */
class Stats {
	/**
	 * LINEメッセージ友達数などの統計データを取得する。
	 */
	public static function fetch_line_message_stats() {
		global $wpdb;
		...
		foreach ( LineConnect::get_all_channels() as $channel_id => $channel ) {
			...
			$dates_to_check       = array( $day_before_yesterday, $yesterday, $today );
	 ...
	 * LINEメッセージ配信数の取得
	 *
	 * @param string $channel_access_token チャネルアクセストークン.
	...
	*/
	public static function get_monthly_summary( $year_month ) {
		...
		foreach ( $message_count_results as $message_count_result ) {
			...
			$summary[ $channel_prefix ] = array_merge( $summary[ $channel_prefix ] ?? array(), $message_count_result );
		}
