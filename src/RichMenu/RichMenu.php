...
EOM;
	...
	*/
	...
	private static function save_richmenu_image($channel, $rich_menu_id, $save_dir = 'richmenu') {
		$target_dir_path = \Shipweb\LineConnect\Utilities\FileSystem::make_lineconnect_dir($save_dir . '/' . $channel['prefix']);
		if ($target_dir_path) {
		...
		if ($isSuccess) {
			...
			if (!empty($channel_prefix) && isset($_POST['richmenu_id'])) {
				...
				if ($channel) {
					...
					// delete cache image
					$target_dir_path = \Shipweb\LineConnect\Utilities\FileSystem::make_lineconnect_dir('richmenu/' . $channel['prefix']);
					if ($target_dir_path) {
