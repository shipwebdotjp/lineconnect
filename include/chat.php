...
class lineconnectChat {
	...
	static function set_plugin_menu() {
		...
		$page_hook_suffix = add_submenu_page(
			...
			array( 'lineconnectChat', 'show_chat' ),
			// メニューの位置
			3
		);
		add_action( "admin_print_styles-{$page_hook_suffix}", array( 'lineconnectChat', 'wpdocs_plugin_admin_styles' ) );
		add_action( "admin_print_scripts-{$page_hook_suffix}", array( 'lineconnectChat', 'wpdocs_plugin_admin_scripts' ) );
	}
	...
	static function show_chat() {
		...
		$line_ids                        = isset( $_GET['line_ids'] ) ? $_GET['line_ids'] : array();
		...
		$toTypeList = array(
			array(
				'name'  => 'broad',
				'label' => __( 'All', lineconnect::PLUGIN_NAME ),
			),
			array(
				'name'  => 'linked',
				'label' => __( 'Linked', lineconnect::PLUGIN_NAME ),
			),
			array(
				'name'  => 'role',
				'label' => __( 'Roles', lineconnect::PLUGIN_NAME ),
			),
			array(
				'name'  => 'multi',
				'label' => __( 'Users', lineconnect::PLUGIN_NAME ),
			),
		);
