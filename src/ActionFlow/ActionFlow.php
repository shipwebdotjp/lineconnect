...
class ActionFlow {
	 ...
	 * @param array $action_hook_args ActionHook からの引数
	 * @return array 実行結果
	 */
	static function execute_actionflow_by_audience( $actionFlow, $recepient, $action_hook_args = array() ) {
		...
		foreach ( $recepient as $secret_prefix => $recepient_item ) {
			...
			if ( $type == 'broadcast' ) {
				...
				foreach ( $recepient_item['line_user_ids'] as $line_user_id ) {
					...
					$action_result         = Action::do_action( $actionFlow['actions'], $actionFlow['chains'] ?? null, $event, $secret_prefix, null, null, $action_hook_args );
					$response              = null;
